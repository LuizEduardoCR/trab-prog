...
class Calculator { //Define a class Calculator
  constructor(previousOperationText, currentOperationText) { //cria objetos
  ...
  addDigit(digit) {//representa o dígito a ser adicionado na operação atual.
  ...
  changeOperation(operation) {
    ...
    if (!mathOperations.includes(operation)) {//ve se o valor pra trocar a operação ta na const mathOperations, se n tiver, retorna um grande nada
      return;
    }
...
document.addEventListener('keydown', (event) => {
    ...
    if (!isNaN(keyValue) || keyValue === '.' || ['+', '-', '*', '/'].includes(keyValue)) {
      ...
      // se a tecla pressionada for 'Enter', processa a operação de igual na calculadora
      ...
      // Se a tecla pressionada for 'Escape', processa a operação de limpar tudo na calculadora
